...
note already contains (prose whitespace aside; code lines must match exactly, trailing spaces included), or that an earlier update of that note already quoted in full, is archived as `noop-*`, only while that note is unchanged
