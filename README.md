...
listed in `_meta/tests/run.sh` as `eval --suite NAME` lines (without the key, on their own state; an unreadable file, no suite or a line it cannot parse turns the gate red). It commits only
