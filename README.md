...
note already contains byte for byte, as whole lines (only trailing spaces of a final prose line are ignored), or whose exact body an earlier update of that note wrote and left intact (the `alambic-body` comment and its quote), is archived as `noop-*`, only while that note is unchanged
