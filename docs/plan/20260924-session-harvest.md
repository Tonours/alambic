...
- Concurrency: `nightly.mjs` covers a refused swap, a late write into a displaced copy (kept, blocks the commit and the next run), an intact copy (never blocks), a held index lock on resume; `review-gate.mjs` covers an atomic save before the archive (draft stays in the inbox, no copy left) and an archive with no free name (source restored).
...
- Last validated state: alambic round 9 fixes, obvault engine sync, brain d844467
- Known failures: none
- Next action: none
