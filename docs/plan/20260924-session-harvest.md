...
- [x] Judge returns `noop` only when the update target already contains the note's whole normalized body (≥ 80 chars); partial overlap stays in the normal path; apply mode archives it as `processed/noop-*` without touching `kb/`, only when the draft still has the bytes the judge read and the update target still has the bytes whose coverage the judge checked, before and after the move (a target changed during the move puts the draft back)
...
- Last validated state: alambic round 11 fixes, obvault engine sync, brain d844467
- Known failures: none
- Next action: none
