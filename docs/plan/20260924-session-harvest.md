...
- [x] `alambic nightly [--push] [--dry-run] [--json]` holds the harvest lock and runs preflight (every mode: clean tracked tree and index, no untracked file in the publishable paths; committing runs: default branch; `--push`: HEAD equal to the fetched upstream, or its own unpushed nightly commit on top of it, which is resumed) → harvest scan → distill (if a distiller is available) → enrich (if `TYPESAFE_API_KEY`) → sidekick apply-all → snapshot → validate + lint + leak-scan → commit, built on a temporary index, only top-level `kb/*` and `ref/*` files, `_meta/enrich-ledger.json` and deletions under `docs/inbox/`, only when every changed path matches the content this run's own steps wrote (write journal) and the tree equals the snapshot the gates checked, and never when the user's index changed → `--push` pushes that exact commit SHA as the one outgoing commit; any red gate means no commit; `--dry-run` never commits; an end-to-end test covers session → distill → review_required → accept → promote → commit → push to a local bare remote → replay without duplicate
...
- Last validated state: alambic round 3 fixes, obvault engine sync, brain 5565923
- Known failures: none
- Next action: none
...
- Nightly concurrent with a human edit.
  - Impact: unintended human edits published. Mitigation: clean tracked tree + index and HEAD == upstream preflight, harvest lock over the whole run, temporary index, write journal (only content the run wrote is committed), snapshot equality after the gates, SHA push.
- brain regression.
