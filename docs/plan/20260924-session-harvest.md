...
- [x] Judge returns `noop` only when the update target already contains the note's whole normalized body (≥ 80 chars); partial overlap stays in the normal path; apply mode archives it as `processed/noop-*` without touching `kb/`, only when the draft still has the bytes the judge read
...
- Last validated state: alambic round 8 fixes, obvault engine sync, brain d844467
- Known failures: none
- Next action: none
