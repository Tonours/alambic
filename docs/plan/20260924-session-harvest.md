...
- [x] `alambic nightly [--push] [--dry-run] [--json]` holds the harvest lock and runs preflight (every mode: clean tracked tree and index, no untracked file in the publishable paths; committing runs: default branch; `--push`: HEAD equal to the fetched upstream, or its own unpushed nightly commit on top of it, which is resumed) → harvest scan → distill (if a distiller is available) → enrich (if `TYPESAFE_API_KEY`) → sidekick apply-all → snapshot → validate + lint + leak-scan → commit, built on a temporary index, only top-level `kb/*` and `ref/*` files, `_meta/enrich-ledger.json` and deletions under `docs/inbox/`, only when every changed path's staged blob ends an unbroken chain of this run's own writes starting at the HEAD blob (write journal: each write records its preimage and result sha256, and a write whose file changed since it was read is refused), when the tree equals the snapshot the gates checked, and with `.git/index.lock` held from the index check to the index refresh, so the user's index cannot change underneath → `--push` pushes that exact commit SHA as the one outgoing commit; any red gate means no commit; `--dry-run` never commits; an end-to-end test covers session → distill → review_required → accept → promote → commit → push to a local bare remote → replay without duplicate (guaranteed for creates and for UPDATEs whose body fits the 600-char excerpt)
...
- Last validated state: alambic round 4 fixes, obvault engine sync, brain 7af1a76
- Known failures: none
- Next action: none
