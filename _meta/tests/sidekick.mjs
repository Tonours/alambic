...
assert((live.next || []).some((line) => line.includes('alambic-sidekick-daily')), 'next names GHA writer')
assert(!(live.next || []).some((line) => /LaunchAgent/i.test(line)), 'next has no LaunchAgent ghost')
...
assert(contract.writer?.kind === 'github-actions', 'contract writer is github-actions')
assert(contract.writer?.id === 'alambic-sidekick-daily', 'contract writer id is GHA workflow')
assert(contract.writer?.workflow === '.github/workflows/alambic-sidekick-daily.yml', 'contract names live workflow')
