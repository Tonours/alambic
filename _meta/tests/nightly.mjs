...
try {
  ...
  const displacedDir = path.join(vault, '.git/alambic-displaced')
  const listDisplaced = () => { try { return fs.readdirSync(displacedDir) } catch { return [] } }
  const swapEnv = { ALAMBIC_WRITE_JOURNAL: path.join(temp, 'swap.jsonl'), ALAMBIC_DISPLACED_DIR: path.join(vault, 'kb') }
  const swapped = path.join(vault, promoted)
  const swapHead = fs.readFileSync(swapped, 'utf8')
  const beforeSwap = listDisplaced()
  ...
  assert.deepEqual(listDisplaced(), beforeSwap, 'a refused swap leaves no displaced copy')
  fs.writeFileSync(swapped, swapHead)
  const kbListed = fs.readdirSync(path.join(vault, 'kb')).sort()
  assert.equal(writeChecked(swapped, swapHead, `${swapHead}\nStep.\n`, swapEnv), true)
  assert.deepEqual(fs.readdirSync(path.join(vault, 'kb')).sort(), kbListed, 'ALAMBIC_DISPLACED_DIR never redirects displaced copies')
  const [copy] = listDisplaced().filter((name) => !beforeSwap.includes(name))
  assert.ok(copy, 'the swap displaced the old file into the git directory')
